...
internal partial class InProcFunc : IRelayTargetFunc
{
	...
	{
		_logger.LogInformation("Executing request {RequestId}", request.RequestId);

		var responseData = new MemoryStream(Encoding.UTF8.GetBytes("{ \"Hello\" : \"World 👋\" }"));

		if (request.BodyContent is not null)
		{
			_logger.LogInformation("Request {RequestId} body size {BodySize}", request.RequestId, request.BodySize);
			...
			{
				_logger.LogInformation("Echo enabled for request {RequestId}", request.RequestId);
...
internal partial class ConnectorService : IHostedService
{
	...
	{
		_logger.LogInformation("Connector starting");
		await _connector.ConnectAsync(cancellationToken);
	}

	public async Task StopAsync(CancellationToken cancellationToken)
	{
		_logger.LogInformation("Connector stopping");
